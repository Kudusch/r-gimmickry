...
v <- function(string) {
  regex <- paste("^", string, "$", sep = "")
  View(JOUR_bereinigt[(grep(regex, colnames(JOUR_bereinigt))):NCOL(JOUR_bereinigt)])
}

getLabels <- function(string) {
  return(attr(JOUR[[string]], "labels"))
}

getName <- function(string) {
  return(as.character(subset(FRAGEN, FRAGEN$name == string)$text))
}
